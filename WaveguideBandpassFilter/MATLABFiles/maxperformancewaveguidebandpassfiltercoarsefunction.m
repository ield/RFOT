function U = maxperformancewaveguidebandpassfiltercoarsefunction(xxx,Bk,xk,zk,f, draw)

%
%%%
%%%%% mapping. In the initial and coarse evaluations, the bk, xk and zk are
%%%%% set to 0 so that there is no need for a new function
%%%
%
zzz=Bk*(xxx-xk)+zk;
...
%%%
%
% The frequency is converted fomr hz to ghz because of the functions used
f = f/1e9;  
[s11,s21] = waveguidebandpassfiltercoarsefunction(zzz, f);
RL=-20*log10(abs(s11));
IL=-20*log10(abs(s21));
...
if draw
    ...
    ylim([0 40])
    title(['Coarse function = ' num2str(U)])
