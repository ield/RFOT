function error=rectifierdiodePULSE(V,f,t,draw)
% RECTIFIERDIODEPULSE computes the current error vector for HB
%   error=RECTIFIERDIODEPULSE(V,f,t,draw)
%
...
% Source
amplitude=1; %volts
omega_p=2*pi*1.5; % 1.5 GHz
t_pulse=12; % ns
t_raise=1; % ns
v3=amplitude*cos(omega_p*t).*...
        ((sin(pi*t/t_raise/2).^2).*(t<t_raise)+...
         (t>t_raise).*(t<t_pulse-t_raise)+...
         (cos(pi*(t-t_pulse+t_raise)/t_raise/2).^2).*...
         (t>t_pulse-t_raise).*(t<t_pulse));
V3=fftuni(v3);
% Circuit elements
RS=50; % ohms
RL=50; % ohms
