function [error,jacobian]=rectifierdiode_Jacob(V,f,t,draw)
% RECTIFIERDIODE_JACOB computes the current error vector for HB and the
%                      jacobian matrix
%   [error,jacobian]=RECTIFIERDIODE_JACOB(V,f,t,draw)
%
...
if nargout > 1
   ...
   jacobianLI=[rYN -iYN;
               iYN  rYN];
   % From the non-linear part...
   di1nldv1=alpha*Isat*exp(alpha*(v1-v2));
   G=fftuni(di1nldv1)/2;G(1)=2*G(1);
   colGdown=G;
   colGup=G;
   GPG=zeros(n_freq,n_freq);
   GMG=zeros(n_freq,n_freq);
   GPG(:,1)=G;
   for ind=2:n_freq
       colGdown=[conj(G(ind));colGdown(1:end-1)];
       colGup=[colGup(2:end);0];
       GPG(:,ind)=colGdown+colGup;
       GMG(:,ind)=colGdown-colGup;
   end
   rGPG=real(GPG);
   iGPG=imag(GPG);
   rGMG=real(GMG);
   iGMG=imag(GMG);
   jacobianNL=[ rGPG -rGPG -iGMG  iGMG;
               -rGPG  rGPG  iGMG -iGMG;
                iGPG -iGPG  rGMG -rGMG;  
               -iGPG  iGPG -rGMG  rGMG];
   % Add linear and non-linear parts                
   jacobian=jacobianLI+jacobianNL;
end
